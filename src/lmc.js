// 保证按需加载的文件路径正确
__webpack_public_path__ = getJsDir('lmc') + '/';
...
function Lmc (file, opts) {
    var that = this;

    if (!file) throw new Error('没有收到图片，可能的解决方案：');
...
Lmc.prototype._getBase64 = function () {
    var that   = this,
        img    = that.img,
        file   = that.file,
        canvas = that.canvas;
...
Lmc.prototype._createBase64ForOldIOS = function () {
    var that        = this,
        img         = that.img,
        canvas      = that.canvas,
...
Lmc.prototype._createBase64 = function () {
    var that        = this,
        resize      = that.resize,
        img         = that.img,
        canvas      = that.canvas,
...
Lmc.prototype._getResize = function () {
    var that        = this,
        img         = that.img,
        defaults    = that.defaults,
...
window.lmc = function (file, opts) {
    return new Lmc(file, opts);
};

// 版本号来自package.json，构建时自动填充
window.lmc.version = '__packageJSON.version__';

module.exports = window.lmc;
